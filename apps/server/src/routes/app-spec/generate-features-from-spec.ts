...
IMPORTANT: Do not ask for clarification. The specification is provided above. Generate the JSON immediately.`;
  ...
  // If Z.ai profile is active, use GLM 4.7 for feature generation
  if (activeProfile === 'zai') {
    model = 'glm-4.7';
    thinkingLevel = undefined;
    logger.info('[FeatureGeneration] Z.ai profile active, using GLM 4.7');
  } else {
  ...
  if (isGlmModel(model)) {
    ...
    logger.info('[FeatureGeneration] Using GLM/Z.ai provider for model:', model);

    const provider = ProviderFactory.getProviderForModel(model);
    const bareModel = stripProviderPrefix(model);

    // For GLM, include explicit instructions to return JSON
