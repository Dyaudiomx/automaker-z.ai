...
import { DEFAULT_PHASE_MODELS, isCursorModel, isGlmModel, stripProviderPrefix } from '@automaker/types';
...
${getStructuredSpecPromptInstruction()}`;
  ...
  // Get model from phase settings, but use GLM 4.7 if Z.ai profile is active
  const settings = await settingsService?.getGlobalSettings();
  const activeProfile = settings?.activeProfile || 'claude';
  
  let model: string;
  let thinkingLevel: 'none' | 'low' | 'medium' | 'high' | 'ultrathink' | undefined;
  
  // If Z.ai profile is active, use GLM 4.7 for spec generation
  if (activeProfile === 'zai') {
    model = 'glm-4.7';
    thinkingLevel = undefined;
    logger.info('[SpecGeneration] Z.ai profile active, using GLM 4.7');
  } else {
  ...
  if (isGlmModel(model)) {
    // Use GLM provider for Z.ai GLM models
    logger.info('[SpecGeneration] Using GLM/Z.ai provider for model:', model);

    const provider = ProviderFactory.getProviderForModel(model);
    const bareModel = stripProviderPrefix(model);

    // For GLM, include the JSON schema in the prompt
    const glmPrompt = `${prompt}

CRITICAL INSTRUCTIONS:
1. DO NOT write any files. DO NOT create any files.
...
Your entire response should be valid JSON starting with { and ending with }. No text before or after.`;
    ...
    for await (const msg of provider.executeQuery({
      ...
      if (msg.type === 'assistant' && msg.message?.content) {
        ...
        if (typeof msg.result === 'string' && msg.result.length > responseText.length) {
    ...
    logger.info(`[GLM] Stream complete. Total response length: ${responseText.length} chars`);
    logger.info(`[GLM] Response preview (first 500 chars): ${responseText.substring(0, 500)}`);
    logger.info(`[GLM] Response preview (last 500 chars): ${responseText.substring(responseText.length - 500)}`);
    
    if (responseText) {
      structuredOutput = extractJson<SpecOutput>(responseText, { logger });
      if (!structuredOutput) {
        logger.error('[GLM] Failed to extract JSON from response. Full response:');
        logger.error(responseText);
      }
    }
  } else if (isCursorModel(model)) {
