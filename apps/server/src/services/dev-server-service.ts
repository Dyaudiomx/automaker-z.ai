...
class DevServerService {
   ...
   * Uses full paths for package managers to work in Electron context
   */
  private async getDevCommand(dir: string): Promise<{ cmd: string; args: string[] } | null> {
    const pm = await this.detectPackageManager(dir);
    if (!pm) return null;

    // Try to find the command using 'which' first, then fall back to known paths
    const findCommand = (name: string): string => {
      // Known paths where package managers are typically installed
      const knownPaths = [
      ...
      for (const fullPath of knownPaths) {
        try {
          ...
          logger.info(`Found ${name} at: ${fullPath}`);
          return fullPath;
        } catch {
      ...
      try {
        ...
        if (foundPath) {
          logger.info(`Found ${name} via which: ${foundPath}`);
          return foundPath;
        }
      } catch {
        // which failed
      ...
      logger.warn(`Could not find ${name}, falling back to bare command`);
      return name;
    };

    const cmd = findCommand(pm);
    logger.info(`Using package manager: ${pm} -> ${cmd}`);

    switch (pm) {
      case 'bun':
        return { cmd, args: ['run', 'dev'] };
      case 'pnpm':
        return { cmd, args: ['run', 'dev'] };
      case 'yarn':
        return { cmd, args: ['dev'] };
  ...
  async startDevServer(
    ...
    const env = {
      ...process.env,
      PORT: String(port),
      PATH: `${additionalPaths}:${currentPath}`,
    };

    // Use full path to package manager directly
    logger.debug(`Full command: ${devCommand.cmd} ${devCommand.args.join(' ')}`);
