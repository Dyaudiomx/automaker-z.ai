...
export class AutoModeService {
  ...
  async executeFeature(
    ...
    try {
      ...
      // Get model from feature, using activeProfile-based default if not specified
      logger.info(
        `🔥🔥🔥 [GLM-DEBUG] Feature ${featureId} raw model value: "${feature.model}" (type: ${typeof feature.model})`
      );
      
      // Get activeProfile from settings to determine default model
      const globalSettings = await this.settingsService?.getGlobalSettings();
      const activeProfile = globalSettings?.activeProfile || 'claude';
      const defaultModel = activeProfile === 'zai' ? 'glm-4.7' : DEFAULT_MODELS.claude;
      logger.info(
        `🔥🔥🔥 [GLM-DEBUG] activeProfile: "${activeProfile}", defaultModel: "${defaultModel}"`
      );
      
      const model = resolveModelString(feature.model, defaultModel);
      logger.info(
        `🔥🔥🔥 [GLM-DEBUG] After resolveModelString: "${model}"`
      );
      const provider = ProviderFactory.getProviderNameForModel(model);
      logger.info(
        `🔥🔥🔥 [GLM-DEBUG] Provider selected: "${provider}" for model: "${model}"`
      );
      logger.info(
  ...
  private async executePipelineSteps(
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      // Get model from feature, using activeProfile-based default
      const pipelineSettings = await this.settingsService?.getGlobalSettings();
      const pipelineActiveProfile = pipelineSettings?.activeProfile || 'claude';
      const pipelineDefaultModel = pipelineActiveProfile === 'zai' ? 'glm-4.7' : DEFAULT_MODELS.claude;
      const model = resolveModelString(feature.model, pipelineDefaultModel);
...
Address the follow-up instructions above. Review the previous work and make the requested changes or fixes.`;
    ...
    // Get model from feature, using activeProfile-based default
    const followUpSettings = await this.settingsService?.getGlobalSettings();
    const followUpActiveProfile = followUpSettings?.activeProfile || 'claude';
    const followUpDefaultModel = followUpActiveProfile === 'zai' ? 'glm-4.7' : DEFAULT_MODELS.claude;
    const model = resolveModelString(feature?.model, followUpDefaultModel);
    const provider = ProviderFactory.getProviderNameForModel(model);
...
`;
    ...
    logger.info(
      `🔥🔥🔥 [GLM-DEBUG-RUNAGENT] runAgent called with model param: "${model}", finalModel from sdkOptions: "${finalModel}"`
    );
    logger.info(
    ...
    logger.info(
      `🔥🔥🔥 [GLM-DEBUG-RUNAGENT] Provider instance: "${provider.getName()}" for finalModel: "${finalModel}" (bare: ${bareModel})`
    );
    logger.info(
