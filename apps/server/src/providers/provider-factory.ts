...
export class ProviderFactory {
  ...
  static getProviderNameForModel(model: string): ModelProvider {
    ...
    console.log(`[ProviderFactory] getProviderNameForModel called with: "${model}" (lower: "${lowerModel}")`);
    ...
    for (const [name, reg] of registrations) {
      const canHandle = reg.canHandleModel?.(lowerModel);
      console.log(`[ProviderFactory] Checking provider "${name}" (priority: ${reg.priority}): canHandle=${canHandle}`);
      if (canHandle) {
        console.log(`[ProviderFactory] Selected provider: "${name}" for model: "${model}"`);
    ...
    for (const [name] of registrations) {
      if (lowerModel.startsWith(`${name}-`)) {
        console.log(`[ProviderFactory] Fallback prefix match: "${name}" for model: "${model}"`);
        return name as ModelProvider;
      }
    }

    // Default to claude (first registered provider or claude)
    console.log(`[ProviderFactory] No provider matched, defaulting to claude for model: "${model}"`);
    return 'claude';
  }
  ...
  static getProviderForModelName(modelId: string): string {
    ...
    console.log(`🔥🔥🔥 [PROVIDER-FACTORY] getProviderForModelName: "${modelId}" (lower: "${lowerModel}")`);
    ...
    console.log(`🔥🔥🔥 [PROVIDER-FACTORY] Registered providers: ${Array.from(providerRegistry.keys()).join(', ')}`);

    // Check each provider's canHandleModel function
    for (const [name, reg] of registrations) {
      const canHandle = reg.canHandleModel?.(lowerModel);
      console.log(`🔥🔥🔥 [PROVIDER-FACTORY] Checking "${name}": canHandle(${lowerModel}) = ${canHandle}`);
      if (canHandle) {
        console.log(`🔥🔥🔥 [PROVIDER-FACTORY] ✅ SELECTED: "${name}" for model: "${modelId}"`);
        return name;
      }
    }
    ...
    for (const [name] of registrations) {
      if (lowerModel.startsWith(`${name}-`)) {
        console.log(`🔥🔥🔥 [PROVIDER-FACTORY] Fallback prefix "${name}" matched for model: "${modelId}"`);
        return name;
      }
    }

    // Default to claude (first registered provider or claude)
    console.log(`🔥🔥🔥 [PROVIDER-FACTORY] ❌ NO MATCH - defaulting to claude for model: "${modelId}"`);
    return 'claude';
  }
...
// Register Z.ai GLM provider
registerProvider('zai', {
  factory: () => new GlmProvider(),
  canHandleModel: (model: string) => model.startsWith('zai:') || model.startsWith('glm-'),
  priority: 4, // Between opencode (3) and claude (0)
});
