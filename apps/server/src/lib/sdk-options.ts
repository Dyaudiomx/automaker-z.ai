...
export function getModelForUseCase(
  ...
  console.log(`🔥🔥🔥 [SDK-OPTIONS] getModelForUseCase: useCase="${useCase}", explicitModel="${explicitModel}"`);
  
  // Explicit model takes precedence
  if (explicitModel) {
    const resolved = resolveModelString(explicitModel);
    console.log(`🔥🔥🔥 [SDK-OPTIONS] Using explicit model: "${explicitModel}" -> resolved: "${resolved}"`);
    return resolved;
  }
