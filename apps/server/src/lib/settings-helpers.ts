...
export async function getPromptCustomization(
  ...
  let activeProfile: 'claude' | 'zai' = 'claude';

  if (settingsService) {
    try {
      const globalSettings = await settingsService.getGlobalSettings();
      customization = globalSettings.promptCustomization || {};
      activeProfile = globalSettings.activeProfile || 'claude';
      logger.info(`${logPrefix} Loaded prompt customization from settings (active profile: ${activeProfile})`);
    } catch (error) {
  ...
  // Merge agent prompts with profile-specific system prompt
  const agentPrompts = mergeAgentPrompts(customization.agent);
  if (activeProfile === 'zai') {
    // Use Z.ai system prompt when Z.ai profile is active
    const { DEFAULT_ZAI_SYSTEM_PROMPT } = await import('@automaker/prompts');
    agentPrompts.systemPrompt = DEFAULT_ZAI_SYSTEM_PROMPT;
  }
