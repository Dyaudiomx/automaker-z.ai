...
export function AgentView() {
  const { currentProject, activeProfile } = useAppStore();
  const [input, setInput] = useState('');
  const [currentTool, setCurrentTool] = useState<string | null>(null);
  const [showSessionManager, setShowSessionManager] = useState(true);
  
  // Use GLM 4.7 as default when Z.ai profile is active, otherwise use sonnet
  const getDefaultModel = (): PhaseModelEntry => {
    return activeProfile === 'zai' ? { model: 'glm-4.7' } : { model: 'sonnet' };
  };
  const [modelSelection, setModelSelection] = useState<PhaseModelEntry>(getDefaultModel);
  
  // Update model selection when activeProfile changes
  useEffect(() => {
    setModelSelection(getDefaultModel());
  }, [activeProfile]);
