...
import { AnthropicIcon, CursorIcon, OpenAIIcon, OpenCodeIcon, ZaiIcon } from '@/components/ui/provider-icon';
...
export function PhaseModelSelector({
  ...
  const currentModel = React.useMemo(() => {
    ...
    if (zaiModel) return { ...zaiModel, icon: ZaiIcon };
  ...
  const { favorites, claude, cursor, codex, opencode, zai } = React.useMemo(() => {
    ...
    const zaiModels: typeof ZAI_MODELS = [];
  ...
  // Render Z.ai model item (simple selector, no thinking/reasoning options)
                ...
                {(() => {
                  ...
                  return favorites.map((model) => {
                    ...
                    if (model.provider === 'zai') {
                      return renderZaiModelItem(model);
                    }
                    // Claude model
