...
export function AddFeatureDialog({
  ...
  const [modelEntry, setModelEntry] = useState<PhaseModelEntry>(() => ({
    model: useAppStore.getState().activeProfile === 'zai' ? 'glm-4.7' as ModelAlias : 'opus'
  }));
  ...
  const { defaultPlanningMode, defaultRequirePlanApproval, defaultAIProfileId, activeProfile } = useAppStore();

  // Get default model based on active profile
  const getDefaultModel = (): ModelAlias => {
    return activeProfile === 'zai' ? 'glm-4.7' as ModelAlias : 'opus';
  };
  ...
  useEffect(() => {
    ...
    if (justOpened) {
      ...
      if (defaultProfile) {
        ...
        setModelEntry({ model: getDefaultModel() });
      }
