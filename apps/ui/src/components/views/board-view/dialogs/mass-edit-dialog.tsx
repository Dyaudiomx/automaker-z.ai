...
import { Feature, ModelAlias, ThinkingLevel, AIProfile, PlanningMode, useAppStore } from '@/store/app-store';
...
export function MassEditDialog({
  ...
  const getDefaultModel = (): ModelAlias => activeProfile === 'zai' ? 'glm-4.7' as ModelAlias : 'sonnet';
  ...
  // Field values
  const [model, setModel] = useState<ModelAlias>(getDefaultModel());
  ...
  useEffect(() => {
    if (open && selectedFeatures.length > 0) {
      ...
      setModel(getInitialValue(selectedFeatures, 'model', getDefaultModel()) as ModelAlias);
