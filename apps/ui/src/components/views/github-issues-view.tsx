...
export function GitHubIssuesView() {
  ...
  const { currentProject, defaultAIProfileId, aiProfiles, getCurrentWorktree, worktreesByProject, activeProfile } =
    useAppStore();
    ...
    async (issue: GitHubIssue, validation: IssueValidationResult) => {
      ...
      try {
        ...
        if (api.features?.create) {
          ...
          // Use profile default model, falling back to activeProfile-based default
          const getDefaultModel = () => activeProfile === 'zai' ? 'glm-4.7' : 'opus';
          const featureModel = defaultProfile?.model ?? getDefaultModel();
