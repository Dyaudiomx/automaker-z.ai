...
import type { ApiKeys } from './app-store';
...
export interface SetupActions {
  ...
  // Z.ai
  setZaiStatus: (status: ZaiStatus | null) => void;
  checkZaiStatus: (apiKeyOverride?: string) => Promise<void>;
...
export const useSetupStore = create<SetupState & SetupActions>()((set, get) => ({
  ...
  // Z.ai
  setZaiStatus: (status) => set({ zaiStatus: status }),

  checkZaiStatus: async (apiKeyOverride?: string) => {
    // Use provided API key or try to get from setup store's cached apiKeys
    const { apiKeys } = get();
    const apiKey = apiKeyOverride || apiKeys?.zai;
    if (!apiKey) {
      set({ zaiStatus: { connected: false, error: 'No API key provided' } });
      return;
    }
    try {
      ...
      if (response.ok) {
        set({ zaiStatus: { connected: true } });
      } else {
        const errorText = await response.text();
        set({ zaiStatus: { connected: false, error: errorText } });
      }
    } catch (error) {
      set({ zaiStatus: { connected: false, error: error instanceof Error ? error.message : 'Unknown error' } });
    }
  },
