...
export interface AppActions {
  ...
  setActiveProfile: (profile: 'claude' | 'zai') => void;
...
const DEFAULT_AI_PROFILES: AIProfile[] = [
  ...
  // Z.ai profiles
  {
    id: 'profile-zai-default',
    name: 'Z.ai Default',
    description: 'GLM 4.7 with advanced reasoning for complex tasks.',
    model: 'glm-4.7',
    thinkingLevel: 'none',
    provider: 'zai',
    isBuiltIn: true,
...
export const useAppStore = create<AppState & AppActions>()((set, get) => ({
  ...
  setActiveProfile: async (profile) => {
    logger.info('setActiveProfile called with profile:', profile);
    set({ activeProfile: profile });
    
    // Update default models based on profile
    if (profile === 'zai') {
      // Switch to Z.ai models
      const zaiModels: PhaseModelConfig = {
        enhancementModel: { model: 'glm-4.7', thinkingLevel: 'none' },
        fileDescriptionModel: { model: 'glm-4.7', thinkingLevel: 'none' },
        imageDescriptionModel: { model: 'glm-4.7', thinkingLevel: 'none' },
        validationModel: { model: 'glm-4.7', thinkingLevel: 'none' },
        specGenerationModel: { model: 'glm-4.7', thinkingLevel: 'none' },
        featureGenerationModel: { model: 'glm-4.7', thinkingLevel: 'none' },
        backlogPlanningModel: { model: 'glm-4.7', thinkingLevel: 'none' },
        projectAnalysisModel: { model: 'glm-4.7', thinkingLevel: 'none' },
        suggestionsModel: { model: 'glm-4.7', thinkingLevel: 'none' },
        memoryExtractionModel: { model: 'glm-4.7', thinkingLevel: 'none' },
      };
      set((state) => ({
        phaseModels: zaiModels,
        enhancementModel: 'glm-4.7' as ModelAlias,
        validationModel: 'glm-4.7' as ModelAlias,
      }));
    } else {
    ...
    // Sync to server settings file
    const { syncSettingsToServer } = await import('@/hooks/use-settings-migration');
    logger.info('Syncing settings to server after profile change');
    const success = await syncSettingsToServer();
    logger.info('Settings sync result:', success);
  },
