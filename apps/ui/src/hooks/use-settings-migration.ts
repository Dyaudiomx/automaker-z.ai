...
/**
 * Load credentials from server and hydrate store
 */
export async function loadCredentialsFromServer(): Promise<void> {
  try {
    const api = getHttpApiClient();
    const result = await api.settings.getCredentialsFull();
    
    if (result.success && result.apiKeys) {
      useAppStore.setState({
        apiKeys: {
          anthropic: result.apiKeys.anthropic || '',
          google: result.apiKeys.google || '',
          openai: result.apiKeys.openai || '',
          zai: result.apiKeys.zai || '',
        },
      });
      logger.info('Credentials loaded from server');
    }
  } catch (error) {
...
export function hydrateStoreFromSettings(settings: GlobalSettings): void {
  ...
  useAppStore.setState({
    ...
    activeProfile: settings.activeProfile ?? 'claude',
