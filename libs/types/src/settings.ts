...
export interface PhaseModelEntry {
  /** The model to use (Claude alias, Cursor model ID, Codex model ID, or Z.ai model ID) */
  model: ModelAlias | CursorModelId | CodexModelId | (typeof import('./model.js').ZAI_MODEL_MAP)[keyof typeof import('./model.js').ZAI_MODEL_MAP];
...
export interface AIProfile {
  ...
  /** Which Z.ai GLM model to use - only for Z.ai provider */
  zaiModel?: string;
...
export function profileHasThinking(profile: AIProfile): boolean {
  ...
  if (profile.provider === 'zai') {
    // Z.ai GLM models support reasoning/thinking
    // GLM 4.7 has built-in reasoning capabilities
    return true;
  }
...
export function getProfileModelString(profile: AIProfile): string {
  ...
  if (profile.provider === 'zai') {
    return `zai:${profile.zaiModel || 'glm-4.7'}`;
  }
...
export interface GlobalSettings {
  ...
  // Profile Selection
  /** Currently selected AI profile: 'claude' or 'zai' */
...
export interface Credentials {
  ...
  apiKeys: {
    ...
    zai: string;
    /** Z.ai Chat API key (for standard subscription) */
    zaiChat?: string;
  };
}
