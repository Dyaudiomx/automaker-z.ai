 ...
 *
 * @param model - Model string to check (e.g., "glm-4.7", "glm-4-plus", "glm-4-flash")
 * @returns true if the model is a Z.ai GLM model
 */
export function isGlmModel(model: string | undefined | null): boolean {
  if (!model || typeof model !== 'string') return false;

  // Check for explicit zai- prefix
  if (model.startsWith(PROVIDER_PREFIXES.zai)) {
    return true;
  }

  // Check for glm- prefix (Z.ai GLM models)
